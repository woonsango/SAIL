...
#         dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, sampler=sampler)
#     else:
...
#         model.freeze_except_vlhead()
#         dataloader = DataLoader(dataset, collate_fn=batch_collate_fn, batch_size=args.batch_size, shuffle=False, num_workers=4, sampler=sampler)
...
#     for epoch in range(args.num_epochs):
#         sampler.set_epoch(epoch)  # 设置epoch以确保数据在每个epoch之间是随机的
#         for batch in tqdm(dataloader):
#             if args.train_data_type == "embedding":
#                 text_embeddings, vision_embeddings = batch
#                 text_embeddings = text_embeddings.to(device)
...
#             gathered_image_embeddings = [torch.zeros_like(norm_vision_embeddings) for _ in range(dist.get_world_size())]
#             gathered_text_embeddings = [torch.zeros_like(norm_text_embeddings) for _ in range(dist.get_world_size())]
#             dist.all_gather(gathered_image_embeddings, norm_vision_embeddings)
#             dist.all_gather(gathered_text_embeddings, norm_text_embeddings)
#             # Concatenate gathered embeddings
#             all_image_embeddings = torch.cat(gathered_image_embeddings)
#             all_text_embeddings = torch.cat(gathered_text_embeddings)
#             all_logits_per_text = torch.matmul(all_text_embeddings, all_image_embeddings.t()) * model.vlhead.logit_scale.exp() + model.vlhead.logit_bias
...
#             logit_bias = model.module.logit_bias.item() if args.train_data_type == "embedding" else model.module.vlhead.logit_bias.item()
#             logit_scale = model.module.logit_scale.exp().item() if args.train_data_type == "embedding" else model.module.vlhead.logit_scale.exp().item()
#             wandb.log({"loss": loss.item(), "epoch": epoch, "lr": scheduler.get_last_lr()[0], "batch_size": args.batch_size, "weight_decay": args.weight_decay, "logit_bias": logit_bias, "logit_scale": logit_scale})
