...
MODEL_CHECKPOINTS = {
    "r101_2x_sk1": "model/backbone_checkpoints/r101_2x_sk1.pth",
    "r152_2x_sk1": "model/backbone_checkpoints/r152_2x_sk1.pth",
}
...
def get_simclr_resnet(model_name):  # sk_ratio=0.0625 is recommended
    assert model_name in MODEL_CHECKPOINTS, f"Model name {model_name} not in MODEL_CHECKPOINTS, Please refer to https://github.com/Separius/SimCLRv2-Pytorch for more details"
    depth, width_multiplier, sk_ratio = name_to_params(model_name)
    layers = {50: [3, 4, 6, 3], 101: [3, 4, 23, 3], 152: [3, 8, 36, 3], 200: [3, 24, 36, 3]}[depth]
    resnet = ResNet(layers, width_multiplier, sk_ratio)
    msg = resnet.load_state_dict(torch.load(MODEL_CHECKPOINTS[model_name])['resnet'])
    print(msg)
    return resnet, ContrastiveHead(resnet.channels_out)
