...
class ImageEmbedding(nn.Module):
    def __init__(
        self, model_name="facebook/dinov2-base", device=None, test: bool = False
    ):
    ...
    def get_visual_embeddings_from_directory(self, images_path: List[str]):
        ...
        inputs = self.image_processor(images, return_tensors="pt").to(
    ...
    def patch_embeddings(self, inputs, skip_res=False):
        ...
        patch_tokens = sequence_output[:, 1:]
        cls_token = (
            sequence_output[:, 0].unsqueeze(1).repeat(1, patch_tokens.shape[1], 1)
        )
