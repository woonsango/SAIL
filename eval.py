from evaluation import imagenet_eval, winoground_eval, coco_eval, update_results_json, extract_info_from_path, check_epoch_exists
import argparse
from model import create_model
import os
def parse_args():
    parser = argparse.ArgumentParser(description='Zero-shot evaluation')
    parser.add_argument('--task', type=str, choices=['imagenet',"coco", "winoground"], default='imagenet', help='Task')
    parser.add_argument(
        "--text-model",
        type=str,
        default=None,
    ...
    parser.add_argument('--results_dir', type=str, default='/home/mila/l/le.zhang/scratch/light_align/evaluation/eval_result', help='Path to results file')
...
def main():
    ...
    output_path = os.path.join(args.results_dir, args.task, model_prefix ,f'{training_info_str}.json')
    ...
    if args.task == 'imagenet':
        results = imagenet_eval(model, bs=args.batch_size)
    elif args.task == 'coco':
        ...
        results = coco_eval(model, bs=args.batch_size, coco_root=coco_root, coco_ann_file=coco_ann_file)
    elif args.task == 'winoground':
        results = winoground_eval(model)
