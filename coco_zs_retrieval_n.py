...
def encode_dataset(clip, dataset: dutils.Dataset, batch_size = 16):
    with torch.no_grad():
        ...
        for text, images in tqdm(dataloader):
            ...
            text_tokens = clip.text_model.tokenizer(text_list, padding=True, truncation=True, return_tensors='pt').to(device) # (B*5) x 77
            ...
            text_encodings.append(clip.encode_text(text_tokens))
