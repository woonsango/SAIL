...
def get_parser_args():
    ...
    parser_eval.add_argument(
        "--dataset_root",
        default="/home/mila/q/qian.yang/scratch/light_align/datasets",
        type=str,
    ...
    parser_eval.add_argument(
        "--output",
        default="{dataset}_{pretrained}_{language}_{task}.json",
        type=str,
...
def main_eval(base):
    ...
    model = create_model(
        text_model_name=base.text_model,
        vision_model_name=base.vision_model,
        head_weights_path=base.head_weights_path,
        linear_type=base.linear_type,
        target_dimension=base.target_dimension,
        device="cuda",
        use_gmp=base.use_gmp,
        gmp_groups=base.gmp_groups,
        test=True,
    )
    # models = [model]
    ...
    runs = product(datasets, languages)
    if base.distributed:
...
def run(args):
    ...
    text_model_image_model = args.text_model + "_" + args.vision_model
    pretrained_slug = text_model_image_model.replace("/", "_")
    ...
    if os.path.exists(output) and args.skip_existing:
        ...
        return
    if args.verbose:
        print(f"Running '{task}' on '{dataset_name}' on language '{args.language}'")
