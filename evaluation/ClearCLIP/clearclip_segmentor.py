...
class ClearCLIPSegmentation(BaseSegmentor):
    def __init__(self, clip_type, vit_type, model_type, name_path, device=torch.device('cuda'), ignore_residual=True,
                 prob_thd=0.0, logit_scale=50, slide_stride=224, slide_crop=448):
    ...
    def forward_feature(self, img, logit_size=None):
        ...
        if self.clip_type == 'BLIP':
            ...
            image_features = self.net.encode_image(img, self.model_type, self.ignore_residual)
        image_features /= image_features.norm(dim=-1, keepdim=True)
        logits = image_features @ self.query_features.T
        # w=h=28, out_dim= num_vocab_classes
    ...
    def forward_slide(self, img, img_metas, stride=112, crop_size=224):
        ...
        # ori_shape can be different from 
